...
export default function PropertiesPage() {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.success(isSw ? `"${deleteTarget.name}" imeondolewa.` : `"${deleteTarget.name}" has been removed.`);
    } catch (err) {
      toast.error(getApiErrorMessage(err, isSw ? 'Imeshindikana kufuta mali.' : 'Failed to delete property.'));
      throw err;
    }
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex justify-between items-end">
        <div>
          <h2 className="text-xl md:text-2xl font-bold text-brand-900 dark:text-white">{t('sidebar.properties')}</h2>
          <p className="text-sm text-brand-500">
            {isSw ? 'Simamia majengo na mali zako za upangishaji' : 'Manage your buildings and real estate assets'}
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {properties.map((p: any) => (
          <div key={p.id} className="glass-panel rounded-2xl p-6 group hover:-translate-y-1 hover:shadow-xl transition-all duration-300">
            <div className="flex justify-between items-start mb-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  title={isSw ? 'Futa mali' : 'Delete property'}
                >
            ...
            <div className="grid grid-cols-2 gap-4 border-t border-brand-100 dark:border-brand-800/50 pt-4 mb-4">
              <div>
                <p className="text-xs text-brand-400 font-semibold uppercase tracking-wider">{isSw ? 'Jumla ya Vyumba' : 'Total Units'}</p>
              ...
              <div>
                <p className="text-xs text-brand-400 font-semibold uppercase tracking-wider">{isSw ? 'Ujazaji' : 'Occupancy'}</p>
                <div className="flex items-center gap-1.5 mt-1">
                ...
                <p className="text-[10px] text-brand-500 mt-0.5">
                  {isSw
                    ? `${p.occupied_count} zimekaliwa, ${p.vacant_count} wazi`
                    : `${p.occupied_count} occupied, ${p.vacant_count} vacant`}
                </p>
              </div>
            </div>
        ...
        {properties.length === 0 && (
          <div className="col-span-full text-center py-20 bg-white/50 dark:bg-brand-900/50 rounded-2xl border border-dashed border-brand-300">
            ...
            <h3 className="text-lg font-bold text-brand-900 dark:text-white">{isSw ? 'Hakuna mali zilizopatikana' : 'No properties found'}</h3>
            <p className="text-brand-500 max-w-sm mx-auto mt-2 mb-6">
              {isSw
                ? 'Bado hujaongeza mali yoyote. Anza kwa kuongeza jengo au apartment.'
                : "You haven't added any properties yet. Start by adding a building or apartment complex."}
            </p>
            <button className="bg-primary text-white px-6 py-2 rounded-lg font-semibold hover:bg-primary/90 transition-colors shadow-sm">
              {isSw ? 'Ongeza Mali ya Kwanza' : 'Add First Property'}
            </button>
          </div>
        )}
      </div>
      ...
      <ConfirmDeleteDialog
        ...
        title={isSw ? 'Futa Mali' : 'Delete Property'}
        description={isSw
          ? 'Una uhakika unataka kufuta mali hii? Itazimwa na haitatokea tena kwenye orodha yako.'
          : 'Are you sure you want to delete this property? This will deactivate it and it will no longer appear in your listings.'}
        itemName={deleteTarget?.name}
        warning={deleteTarget?.occupied_count > 0
          ? (isSw
            ? `Mali hii ina chumba ${deleteTarget.occupied_count} kilichokaliwa. Lazima viwe wazi kabla ya kufuta.`
            : `This property has ${deleteTarget.occupied_count} occupied unit(s). They must be vacated before deletion.`)
          : undefined}
      />
    </div>
  );
