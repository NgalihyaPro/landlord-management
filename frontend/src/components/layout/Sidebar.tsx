...
const DATA_PREFETCHERS: Record<string, () => void> = {
  ...
  '/payments': () => {
    prefetchGet('/payments');
    prefetchGet('/payments/methods');
  },
...
export function Sidebar() {
  ...
  useEffect(() => {
    ...
    SECONDARY_LINKS.forEach((link) => {
      if (link.platformAdminOnly && !user?.is_platform_admin) {
        return;
      }
  ...
  return (
    <div className="flex h-screen w-64 flex-col justify-between border-r border-border bg-white shadow-sm dark:bg-brand-900 glass-panel">
      <div>
        <div className="flex h-16 items-center px-6 font-bold text-2xl tracking-tight text-primary">
          <BuildingOfficeIcon className="mr-2 h-8 w-8 text-primary" />
          LandlordPro
        </div>
        
        <div className="mt-6 flex flex-col gap-1 px-4">
          <p className="px-2 text-xs font-semibold text-brand-400 mb-2 uppercase tracking-wider">{t('sidebar.menu_title')}</p>
      ...
      <div className="border-t border-border p-4">
        ...
        {user && (
          <div className="mt-6 flex items-center gap-3 rounded-xl bg-brand-50 p-3 dark:bg-brand-800">
